...
pub struct ConfigBuilder<Side: ConfigSide, State> {
    ...
    // XXX TBD XXX XXX
    // pub(crate) provider_ref: &'static alloc::boxed::Box<CryptoProvider>,
    pub(crate) provider: &'static alloc::boxed::Box<CryptoProvider>,
...
impl<Side: ConfigSide, State> ConfigBuilder<Side, State> {
    ...
    pub fn crypto_provider(&self) -> &alloc::boxed::Box<CryptoProvider> {
        &self.provider
    }
}
